...
public class Chemin {
	...
	public ArrayList<Point> getKrus() {
		return krus;
	}
	...
	public void initDistances(int n) {
		...
		for(int i = 0; i < n; i++) {
			for(int j = i+1; j < n; j++) {			
				this.edge.add(new EdgeElement(listePoint.get(i),listePoint.get(j), listePoint.get(i).distance(listePoint.get(j))));
				k++;
			}
		}
	...
	private Integer findPoint(Point p) {
		...
		for (Integer i = 0; i < listePoint.size(); i++) {
			System.out.println("le point nn touve :" + p + "le point compar� : " + listePoint.get(i));
		}
		return -1;
	...
	public ArrayList<Point> parcours(ArrayList<Point> kruskal) {
		...
		//sommets deja vu : dv initialis� � 0
		for(int i = 0; i < listePoint.size(); i++) {
